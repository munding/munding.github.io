...
| vim +/pattern filename   | 打开文件,并将光标置于第一一个 与pattern匹配的串处 |
| vim -r filename          | 在上次正用vim编辑时发生系统崩溃，恢复 flename     |
...
| nomagic                   | 允许vi显示其他用户用write写到自己终端上的信息             |
| nowrapscan                | 禁止vi在搜索到达文件两端时，又从另- -端开始               |
...
| :set softabstop=8         | 4:4个空格,8:正常的制表符，12:一一个制表符4个空格，16:两个 |
| :set autoindent           | 自动缩进                                                  |
...
| :1,$ s/string1/string2/g | 在全 文中将string1替换为string2                      |
...
| 在所有行插入相同的内容如include< | 将光标移到开始插入的位置，按CTRL+V进入VISUAL模式 ,<br />选择好模块后按I（shift+i），后插入要插入的文本，按[ESC]完成 |
