...
description: "快速搭建一个企业知识库，Confluence 是一个不错的选择"
summary: ""
draft: false
---
...
默认内存分配为 1024m，如果需要覆盖 Confluence Server 的默认内存分配，可以通过环境变量 `JVM_MINIMUM_MEMORY`、`JVM_MAXIMUM_MEMORY`、`JVM_RESERVED_CODE_CACHE_SIZE` 控制最小堆 (Xms) 和最大堆(Xmx)。
